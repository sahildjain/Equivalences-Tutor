		...
		try {
			...
			if(node instanceof ASTOrNode) {
				...
				if(equivalence.isEquivalent()) {
					ASTNode propNode = orNode.getLeft();
	...
	public AST associativityLeft() {
		try {
			...
			if(node instanceof ASTOrNode) {
				...
				ASTNode left = orNode.getLeft();
				ASTNode right = orNode.getRight();
				ASTNode newLeft = ((ASTOrNode) left).getLeft();
				ASTNode newRight = new ASTOrNode(tree.getKey(), ((ASTOrNode) left).getRight(), right);
				tree.setKey(tree.getKey() + 1);
				ASTOrNode newOrNode = new ASTOrNode(tree.getKey(), newLeft, newRight);
				tree.setKey(tree.getKey() + 1);
				ASTNode p = replace(tree.getRoot().getLeaf(), newOrNode, key);
				if(p == null) {
					return null;
				}
				ASTProgramNode program = tree.getRoot();
	...
	public AST associativityRight() {
		try {
			...
			if(node instanceof ASTOrNode) {
				...
				ASTNode left = orNode.getLeft();
				ASTNode right = orNode.getRight();
				ASTNode newRight = ((ASTOrNode) right).getRight();
